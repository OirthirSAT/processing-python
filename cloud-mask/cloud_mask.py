...
class CloudMask:
    ...
    def __readfile(self, file: str, downsample_factor: float) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
        ...
        image_resized: NDArray[np.uint8] = cv2.resize(image_bgr, new_size, interpolation=cv2.INTER_AREA)
    ...
    def __compute_ndsi(self) -> NDArray[np.floating[Any]]:
        ...
        return cast(
            ...
            (self.__band_green.astype(float) - self.__band_nir.astype(float)) / (self.__band_green.astype(float) + self.__band_nir.astype(float)),
        )
    ...
    def __compute_brightness(self) -> NDArray[np.floating[Any]]:
        ...
        return cast(
            NDArray[np.floating[Any]],
            (self.__band_red.astype(float) + self.__band_green.astype(float) + self.__band_blue.astype(float) + self.__band_nir.astype(float)) / 4 / 256,
        )
    ...
    def apply_cloud_mask(self, cloud_mask: NDArray[np.bool_]) -> NDArray[np.floating[Any]]:
        ...
        masked_image = np.array(np.dstack(bands), dtype=float) # Shape: (rows, columns, bands). Convert to float to allow NaN values.
    ...
    def visualise_image(image: NDArray[np.uint8]):
        ...
        plt.imshow(image.astype(np.uint8)[:,:,:3]) # Only visually render RGB channels
        plt.show()
